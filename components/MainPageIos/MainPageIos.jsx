...
export const findCoffee = (text) => {
  for (let key in coffeeMenu) {
    const result = coffeeMenu[key].filter((elem) => elem.name.startsWith(text));
    if (result.length) {
      return result;
    }
  }
...
export default function MainPageIos() {
  ...
  const onHandlePress = () => {
    if (InputText) {
      const res = findCoffee(InputText, coffeeMenu);
      setfindElem(res);
    }
  };
      ...
      <SafeAreaView>
        ...
        <View style={styles.menu}>
          ...
          <View style={styles.coffeeList}>
            <View>
              {Object.keys(coffeeMenu).map((item, index) => {
                return (
                  <View key={item}>
                    <RobotoText
                      weight="bold"
